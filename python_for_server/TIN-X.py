...
# Output CSV files:
PROTEIN_NOVELTY_FILE = '../data/TIN-X/ROSEv%s/ProteinNovelty.csv'%BIRD_VER
DISEASE_NOVELTY_FILE = '../data/TIN-X/ROSEv%s/DiseaseNovelty.csv'%BIRD_VER
PMID_RANKING_FILE = '../data/TIN-X/ROSEv%s/PMIDRanking.csv'%BIRD_VER
IMPORTANCE_FILE = '../data/TIN-X/ROSEv%s/Importance.csv'%BIRD_VER

def process_chunk(lines, do, doid2pmids, pmid_disease_ct):
    notfnd = set()
    for line in lines:
        ...
        if doid not in do:
            logger.warning("%s not found in DO" % doid)
            notfnd.add(doid)
            continue
...
def load(args,dba):
    ...
    with open(fn, 'r') as tsvf:
        ...
        for line in tsvf:
           ...
           if len(lines) >= CHUNK_SIZE:
               chunk_notfnd = process_chunk(lines, do, doid2pmids, pmid_disease_ct)
               notfnd.update(chunk_notfnd)
               lines = []  # Clear the lines list to free up memory
    ...
    if notfnd:
        print("WARNING: No entry found in DO map for {} DOIDs. See logfile {} for details.".format(len(notfnd), logfile))
