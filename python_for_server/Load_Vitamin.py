...
Usage:
    load-Vitamin.py [--debug | --quiet] [--dbhost=<str>] [--dbname=<str>] [--pwfile=<str>] [--dbuser=<str>] [--logfile=<file>] [--loglevel=<int>]
    load-Vitamin.py -? | --help
...
def load(args, dba, dataset_id, logger, logfile):
    ...
    with open(Vitamin_csv_file,'r') as ifh:
        ...
        for row in csvreader:
            ...
            if not pids:
                notfnd.add(f"{up}")
                logger.warning(f"No protein found for {up}")
