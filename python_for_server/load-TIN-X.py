...
PROTEIN_NOVELTY_FILE = '../data/TIN-X/TCRDv%s/ProteinNovelty.csv'%ROSE_VER
DISEASE_NOVELTY_FILE = '../data/TIN-X/TCRDv%s/DiseaseNovelty.csv'%ROSE_VER
PMID_RANKING_FILE = '../data/TIN-X/TCRDv%s/PMIDRanking.csv'%ROSE_VER
IMPORTANCE_FILE = '../data/TIN-X/TCRDv%s/Importance.csv'%ROSE_VER
...
def load(args, dba, dataset_id, logger, logfile):
    ...
    with open(DISEASE_NOVELTY_FILE, 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            if doid in do:
                ...
                logger.warning("%s not in DO map" % row[0])
                notfnd.append(row[0])
                continue
    ...
    with open(PMID_RANKING_FILE, 'rU') as csvfile:
        ...
        for row in csvreader:
            ...
            k = "%s|%s"%(row[0],row[1])
            if k not in imap:
                logger.warning("%s not in imap" % k)
                skips.add(k)
                continue
            iid = imap[k]
