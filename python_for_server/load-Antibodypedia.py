...
def load(args, dba, dataset_id, logger, logfile):
    ...
    for protein in dba.get_proteins():
        ...
        while attempts <= 5:
            try:
                logger.info(f"getting {url} [target {pid}, attempt {attempts}]")
                r = requests.get(url)
                break
            except:
                attempts += 1
        ...
        if r.status_code != 200:
            net_err_ct += 1
            logger.error(f"Bad response: {r.status_code} for {url} [target {pid}, attempts {attempts}]")
            continue
...
if __name__ == '__main__':
    ...
    dataset_id = dba.ins_dataset({'name': 'Antibodypedia.com', 'source': "Web API at http://www.antibodypedia.com/tools/antibodies.php?uniprot=", 'app': PROGRAM, 'app_version': __version__, 'url': 'http://www.antibodypedia.com'})
