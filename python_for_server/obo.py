...
class Parser(object):
  ...
  def __init__(self, infile, encoding='utf-8'):
    """Creates an OBO parser that processes the input file.
    ...
    """
    self.file_handle = open(infile, 'r', encoding=encoding)
...
def test():
  for f in ['/home/app/TCRD/data/eco.obo', '/home/app/TCRD/data/DiseaseOntology/doid.obo']:
    ...
    for _ in parser:
      ct += 1
      if ct % 100 == 0:
        print(f"  {ct} stanzas processed")
    print(f"Successfully parsed {ct} stanzas")
