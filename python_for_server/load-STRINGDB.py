...
INFILE = '../data/StringIDS/9606.protein.links.v11.5.txt'
        ...
        for line in ifh:
            ...
            if ensp1 in ensp2pids:
                ...
                if not targets:
                    notfnd.add(ensp1)
                    continue
                p1s = [(t['id'], t['sym']) for t in targets]
                ensp2pids[ensp1] = p1s
            if ensp2 in ensp2pids:
                ...
                if not targets:
                    notfnd.add(ensp2)
                    continue
                p2s = [(t['id'], t['sym']) for t in targets]
                ensp2pids[ensp2] = p2s
            for p1 in p1s:
                for p2 in p2s:
                    if p1[0] == p2[0]:
...
if __name__ == '__main__':
    ...
    dataset_id = dba.ins_dataset({
        'name': 'STRINGDB',
        'source': 'File %s from https://stringdb-static.org/download/protein.links.v11.5/' % os.path.basename(INFILE),
        'app': PROGRAM, 'app_version': __version__, 'url': 'http://string-db.org/'
    })
